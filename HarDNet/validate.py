    ...
    for i, (images, labels, fname) in enumerate(valloader):
        ...
        if args.eval_flip:
            ...
            if args.save_image:
                ...
                dir = Path(os.getcwd(), "pic", "out_predID")
                if not dir.exists():
                ...
                if save_rgb:
                    ...
                    fname_new += '.jpg'
                    dir = Path(os.getcwd(), "pic", "out_rgb")
                    if not dir.exists():
                        dir.mkdir(parents=True)
        ...
        acc = np.sum(gt == pred) / (cfg['data']['img_rows'] * cfg['data']['img_cols'])

        if args.measure_time:
            total_time += elapsed_time
            print(
                "Inference time \
                  (iter {0:5d}): acc={1:4f}, time={2:3.5f} fps".format(
                    i + 1, acc, 1 / elapsed_time
                )
            )
