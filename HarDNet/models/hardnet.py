...
class HarDBlock(nn.Module):
    def get_link(self, layer, base_ch, growth_rate, grmul):
        """
        Creating links between sub layers in HardBlock.
        @link: https://arxiv.org/pdf/1909.00948.pdf figure 2.
        @param layer: Current layer
        @param base_ch: block input size
        @param growth_rate:
        @param grmul: m
        @return:
        """
        ...
        for i in range(10):
            ...
            if layer < dv:
                break
            if layer % dv == 0:  # get link from all the previous keyframe or get link to previous layer (i==0)
                k = layer - dv
                link.append(k)
                if i > 0:
                    out_channels *= grmul
    ...
    def __init__(self, in_channels, growth_rate, growth_multiplier, n_layers, keepBase=False, residual_out=False):
        """
        @param in_channels:
        @param growth_rate: A layer l has an initial growth rate K
        @param growth_multiplier: m
        @param n_layers: is how many sub convolutional layers within this HarDBlock Module.
        @param keepBase:
        @param residual_out: Not used.
        """
        super().__init__()
        self.in_channels = in_channels
        self.growth_rate = growth_rate
        self.grmul = growth_multiplier
        ...
        for i in range(n_layers):
            outch, inch, link = self.get_link(i + 1, in_channels, growth_rate, growth_multiplier)
            self.links.append(link)
            use_relu = residual_out
            layers_.append(ConvLayer(inch, outch))
            """
            paper quote: In HDB with depth L, the gradient will pass through at most log L layers.
            To alleviate the degradation, we made the output of a depth-L HDB to be
            the concatenation of layer L and all its preceding odd numbered layers, 
            which are the least significant layers with k output channels.
            """
            is_last_layer = (i == n_layers - 1)
            odd_number_layer = (i % 2 == 0)  # Since we start from zero its the even number layers
            if odd_number_layer or is_last_layer:
