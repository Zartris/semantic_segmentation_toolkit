...
from ptsemseg.loss import get_loss_class
...
def train(cfg, logger, logdir):
    ...
    optimizer = optimizer_cls(params_list, **optimizer_params)
    print("Using optimizer {}".format(optimizer))

    scheduler = get_scheduler(optimizer, cfg["training"]["lr_schedule"], max_iterations=cfg["training"]["train_iters"])

    criteria_pre = get_loss_class(cfg)
    criteria_aux = [get_loss_class(cfg) for _ in range(cfg['training']['num_aux_heads'])]
    print("Using loss {}".format(criteria_pre))
    ...
    loss_pre_meter = averageMeter('train_loss_prem')
    loss_aux_meters = [averageMeter('train_loss_aux{}'.format(i))
                       for i in range(cfg['training']['num_aux_heads'])]
    time_meter = averageMeter("timer")

    index_to_category_name = [k for k in data_loader.categories.keys()]

    i = start_iter
    if hasattr(criteria_pre, 'epoch'):
        criteria_pre.epoch = start_iter
        for c in criteria_aux:
            c.epoch = start_iter
    flag = True
    loss_all = 0
    loss_n = 0
    while i <= cfg["training"]["train_iters"] and flag:
        ...
        for i_train_batch, (images, labels, _) in enumerate(trainloader):
            ...
            # combined_img_list = convert_images(writer, trainloader, images.data.cpu().numpy(), pred, labels.data.cpu().numpy(),
            #                                    i_train_batch, i + 1, combined_img_list, "train")
            ...
            if (i + 1) % cfg["training"]["print_interval"] == 0:
                fmt_str = "Iter [{:d}/{:d}]  Loss: {:.4f}, Loss_pre: {:.4f}, Loss_aux: {}, Time/Image: {:.4f}  lr={:.6f}"
                print_str = fmt_str.format(
                    i + 1,
                    cfg["training"]["train_iters"],
                    loss_meter.avg,
                    loss_pre_meter.avg,
                    ''.join(['{}: {:.4f} '.format(el.name, el.avg) for el in loss_aux_meters]),
                ...
                writer.add_scalar("loss_pre/train_loss", loss_pre.item(), i + 1)
                for j, la in enumerate(loss_aux):
                    writer.add_scalar("loss_aux_" + str(j) + "/train_loss", la.item(), i + 1)
                time_meter.reset()
            ...
            if (i + 1) % cfg["training"]["val_interval"] == 0 or (i + 1) == cfg["training"]["train_iters"]:
                ...
                with torch.no_grad():
                    ...
                    for i_val, (images_val, labels_val, _) in tqdm(enumerate(valloader)):
                        ...
                        with amp.autocast(enabled=cfg['training']['use_fp16']):
                            logits, *logits_aux = model(images_val)
                            val_loss_pre = criteria_pre(input=logits, target=labels_val)
                            val_loss_aux = [crit(lgt, labels_val) for crit, lgt in zip(criteria_aux, logits_aux)]
                            val_loss = val_loss_pre + sum(val_loss_aux)
                ...
                writer.add_scalar("loss/valwriter_loss", val_loss_meter.avg, i + 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        default="configs/BiSeNetv2_dry.yml",
