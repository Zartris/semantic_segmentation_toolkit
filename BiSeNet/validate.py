...
def validate(cfg, args):
    ...
    for i, (images, labels, fname) in enumerate(valloader):
        ...
        if args.eval_flip:
            ...
            if args.save_image:
                pred = np.squeeze(outputs.cpu().numpy(), axis=0)
                save_rgb = True
                ...
                if not os.path.exists(dir):
                    os.mkdir(dir)
                plt.imsave(dir + fname[0], decoded)
                ...
                if save_rgb:
                    ...
                    if not os.path.exists(dir):
                        os.mkdir(dir)
                    plt.imsave(dir + fname_new, blend)
    ...
    running_metrics.plot_conf_matrix("??", index_to_category_name, percentage=True,
                                     ...
                                     save_image="F:\\code\\python\\Hardnet\\BiSeNet\\runs\\BiSeNetv2_dry\\cur\\test\\")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        default="F:\\code\\python\\Hardnet\\BiSeNet\\configs\\BiSeNetv2_dry.yml",
