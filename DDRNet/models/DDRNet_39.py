...
class DualResNet(nn.Module):
    ...
    def __init__(self, n_classes, block, layers, planes=64, spp_planes=128, head_planes=128, augment=False):
        ...
        if self.augment:
            self.seghead_extra = segmenthead(highres_planes, head_planes, n_classes)

        self.final_layer = segmenthead(planes * 4, head_planes, n_classes)
...
def DualResNet_imagenet(pretrained=False):
    model = DualResNet(n_classes=19, block=BasicBlock, layers=[3, 4, 6, 3], planes=64, spp_planes=128, head_planes=256,
                       augment=False)
    if pretrained:
